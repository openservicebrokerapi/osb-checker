...
func pollInstanceLastOperationStatus(instanceID string, operation string) error {
	return pollLastOperationStatus(instanceID, "", operation)
}
...
func pollLastOperationStatus(instanceID, bindingID string, operation string) error {
		...
		body, resp, err := func() (openapi.LastOperationResource, *http.Response, error) {
			if bindingID == "" {
				return cli.ServiceInstancesApi.ServiceInstanceLastOperationGet(
					...
					&openapi.ServiceInstanceLastOperationGetOpts{Operation: optional.NewString(operation)})
			} else if operation == "" {
